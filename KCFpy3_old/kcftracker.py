...
def rearrange(img):
    ...
    img_[0:yh,0:xh], img_[yh:int(img.shape[0]),xh:int(img.shape[1])] = img[yh:int(img.shape[0]),xh:int(img.shape[1])], img[0:yh,0:xh]
    img_[0:yh,xh:int(img.shape[1])], img_[yh:int(img.shape[0]),0:xh] = img[yh:int(img.shape[0]),0:xh], img[0:yh,xh:int(img.shape[1])]
    return img_
...
def subwindow(img, window, borderType=cv2.BORDER_CONSTANT):
    ...
    res = img[int(cutWindow[1]):int(cutWindow[1] + cutWindow[3]), int(cutWindow[0]):int(cutWindow[0] + cutWindow[2])]
...
class KCFTracker:
    ...
    def getFeatures(self, image, inithann, scale_adjust=1.0):
        ...
        if(z.shape[1] != self._tmpl_sz[0] or z.shape[0] != self._tmpl_sz[1]):
            z_size = tuple([int(s) for s in self._tmpl_sz])
            z = cv2.resize(z, z_size)
